...
class RobotTestRunner:
    ...
    def run_test(self, license_plate=None, serial=None, headless=False):
        ...
        # Add robot file
        robot_file = self.root_dir / "robots" / "stampcar-be8.robot"
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Robot Framework tests with enhanced logging"
    )
    ...
    parser.add_argument(
        "--root-dir", default=".", help="Root directory for output files"
    )
    ...
    exit_code = runner.run_test(
        license_plate=args.license, serial=args.serial, headless=args.headless
    )
